...
const Services: React.FC = () => {
  ...
  return (
    <section id="services" className="py-16 bg-gradient-to-br from-[#0A2540] via-[#0A2540] to-[#1a4a6e] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-white/5 rounded-xl p-6 animate-pulse h-64 border border-white/10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {servicesToDisplay.map((service) => (
              <div 
                key={service.id} 
                className="group bg-white/5 border border-white/10 rounded-xl p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 backdrop-blur-sm"
              >
                ...
                <Link 
                  ...
                  className="inline-flex items-center text-sm font-medium text-[#00CFFF] hover:text-white transition-colors"
                >
                  Learn more
